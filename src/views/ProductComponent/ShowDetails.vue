  ...
  <div class="container">
    <div class="row pt-5">
      ...
      <div class="col-md-4 col-12">
        <img :src="ProductComponent.imageURL" :alt="ProductComponent.name" class="img-fluid" />
      </div>
      <div class="col-md-6 col-12 pt-3 pt-md-0">
        <h4>{{ ProductComponent.name }}</h4>
        <h6 class="category font-italic">{{ category.categoryName }}</h6>
        <h6 class="font-weight-bold">$ {{ ProductComponent.price }}</h6>
        <p>
        ...
        <button
          ...
          :class="{ ProductComponent_added_wishlist: isAddedToWishlist }"
...
export default {
  ...
  methods: {
    addToWishList(ProductComponentId) {
      axios
      ...
      axios
        ...
        .then(
          (response) => {
            if (response.status == 201) {
              swal({
                text: "ProductComponent Added to the cart!",
                icon: "success",
  ...
  mounted() {
    ...
    this.category = this.categories.find(
      (category) => category.id == this.ProductComponent.categoryId
    );
